"use strict";

var _interopRequireWildcard = require("@babel/runtime/helpers/interopRequireWildcard");

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = exports.components = exports.LazySortableItem = exports.LazySortableGroupComponent = void 0;

var _core = require("@emotion/core");

var _classCallCheck2 = _interopRequireDefault(require("@babel/runtime/helpers/classCallCheck"));

var _createClass2 = _interopRequireDefault(require("@babel/runtime/helpers/createClass"));

var _possibleConstructorReturn2 = _interopRequireDefault(require("@babel/runtime/helpers/possibleConstructorReturn"));

var _getPrototypeOf3 = _interopRequireDefault(require("@babel/runtime/helpers/getPrototypeOf"));

var _assertThisInitialized2 = _interopRequireDefault(require("@babel/runtime/helpers/assertThisInitialized"));

var _inherits2 = _interopRequireDefault(require("@babel/runtime/helpers/inherits"));

var _defineProperty2 = _interopRequireDefault(require("@babel/runtime/helpers/defineProperty"));

var _objectSpread2 = _interopRequireDefault(require("@babel/runtime/helpers/objectSpread"));

var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/extends"));

var _css2 = _interopRequireDefault(require("@emotion/css"));

var _objectWithoutProperties2 = _interopRequireDefault(require("@babel/runtime/helpers/objectWithoutProperties"));

var _react = _interopRequireWildcard(require("react"));

var _reactLoadable = _interopRequireDefault(require("react-loadable"));

var _constants = require("@atlaskit/theme/constants");

var _analytics = require("../common/analytics");

var _RenderBlocker = _interopRequireDefault(require("../components/common/RenderBlocker"));

var _ContainerHeader = _interopRequireDefault(require("../components/presentational/ContainerHeader"));

var _Group = _interopRequireDefault(require("../components/presentational/Group"));

var _GroupHeading = _interopRequireDefault(require("../components/presentational/GroupHeading"));

var _HeaderSection = _interopRequireDefault(require("../components/presentational/HeaderSection"));

var _MenuSection = _interopRequireDefault(require("../components/presentational/MenuSection"));

var _Section = _interopRequireDefault(require("../components/presentational/Section"));

var _SectionHeading = _interopRequireDefault(require("../components/presentational/SectionHeading"));

var _Separator = _interopRequireDefault(require("../components/presentational/Separator"));

var _Wordmark = _interopRequireDefault(require("../components/presentational/Wordmark"));

var _BackItem = _interopRequireDefault(require("../components/connected/BackItem"));

var _ConnectedItem = _interopRequireDefault(require("../components/connected/ConnectedItem"));

var _GoToItem = _interopRequireDefault(require("../components/connected/GoToItem"));

var gridSize = (0, _constants.gridSize)();

var loadSwitcher = function loadSwitcher() {
  return Promise.resolve().then(function () {
    return require('../components/presentational/Switcher');
  });
};

var LazySwitcher = (0, _reactLoadable.default)({
  loader: loadSwitcher,
  loading: function loading() {
    return null;
  }
});

var loadSortableContextComponent = function loadSortableContextComponent() {
  return Promise.resolve().then(function () {
    return require('../components/connected/SortableContext');
  });
};

var LazySortableContextComponent = (0, _reactLoadable.default)({
  loader: loadSortableContextComponent,
  loading: function loading() {
    return null;
  }
});

var loadSortableGroupComponent = function loadSortableGroupComponent() {
  return Promise.resolve().then(function () {
    return require('../components/connected/SortableGroup');
  });
};

var LazySortableGroupComponent = (0, _reactLoadable.default)({
  loader: loadSortableGroupComponent,
  loading: function loading() {
    return null;
  }
});
exports.LazySortableGroupComponent = LazySortableGroupComponent;

var loadSortableItem = function loadSortableItem() {
  return Promise.resolve().then(function () {
    return require('../components/connected/SortableItem');
  });
};

var LazySortableItem = (0, _reactLoadable.default)({
  loader: loadSortableItem,
  loading: function loading() {
    return null;
  }
});
/**
 * ITEMS
 */
// Title

exports.LazySortableItem = LazySortableItem;

var GroupHeading = function GroupHeading(_ref) {
  var text = _ref.text,
      props = (0, _objectWithoutProperties2.default)(_ref, ["text"]);
  return (0, _core.jsx)(_GroupHeading.default, props, text);
}; // SectionHeading


var SectionHeading = function SectionHeading(_ref2) {
  var text = _ref2.text,
      props = (0, _objectWithoutProperties2.default)(_ref2, ["text"]);
  return (0, _core.jsx)(_SectionHeading.default, props, text);
}; // ContainerHeader


var ContainerHeader = function ContainerHeader(props) {
  return (// -2px here to account for the extra space at the top of a MenuSection for
    // the scroll hint.
    (0, _core.jsx)("div", {
      css:
      /*#__PURE__*/
      (0, _css2.default)({
        paddingBottom: gridSize * 2.5 - 2
      }, process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */")
    }, (0, _core.jsx)(_ContainerHeader.default, props))
  );
};

var Debug = function Debug(props) {
  return (0, _core.jsx)("pre", {
    css:
    /*#__PURE__*/
    (0, _css2.default)({
      backgroundColor: 'rgba(0, 0, 0, 0.1)',
      fontSize: '10px',
      overflowX: 'auto',
      padding: "".concat(gridSize / 2, "px")
    }, process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */")
  }, JSON.stringify(props, null, 2));
};
/**
 * GROUPS
 */
// Group


var Group = function Group(_ref3) {
  var customComponents = _ref3.customComponents,
      hasSeparator = _ref3.hasSeparator,
      heading = _ref3.heading,
      items = _ref3.items,
      id = _ref3.id;
  return items.length ? (0, _core.jsx)(_Group.default, {
    heading: heading,
    hasSeparator: hasSeparator,
    id: id
  }, (0, _core.jsx)(TypedItemsRenderer, {
    items: items,
    customComponents: customComponents
  })) : null;
};

var SortableGroup = function SortableGroup(_ref4) {
  var customComponents = _ref4.customComponents,
      hasSeparator = _ref4.hasSeparator,
      heading = _ref4.heading,
      items = _ref4.items,
      id = _ref4.id;
  return items && items.length ? (0, _core.jsx)(LazySortableGroupComponent, {
    heading: heading,
    hasSeparator: hasSeparator,
    id: id
  }, (0, _core.jsx)(_RenderBlocker.default, {
    items: items,
    customComponents: customComponents
  }, (0, _core.jsx)(TypedItemsRenderer, {
    items: items,
    customComponents: customComponents
  }))) : null;
}; // Section


var Section = function Section(_ref5) {
  var _ref5$alwaysShowScrol = _ref5.alwaysShowScrollHint,
      alwaysShowScrollHint = _ref5$alwaysShowScrol === void 0 ? false : _ref5$alwaysShowScrol,
      customComponents = _ref5.customComponents,
      id = _ref5.id,
      items = _ref5.items,
      nestedGroupKey = _ref5.nestedGroupKey,
      parentId = _ref5.parentId,
      shouldGrow = _ref5.shouldGrow;
  return items.length ? (0, _core.jsx)(_Section.default, {
    alwaysShowScrollHint: alwaysShowScrollHint,
    id: id,
    key: nestedGroupKey,
    parentId: parentId,
    shouldGrow: shouldGrow
  }, function (_ref6) {
    var className = _ref6.className;
    return (0, _core.jsx)("div", {
      className: className
    }, (0, _core.jsx)(TypedItemsRenderer, {
      items: items,
      customComponents: customComponents
    }));
  }) : null;
};

var HeaderSection = function HeaderSection(_ref7) {
  var customComponents = _ref7.customComponents,
      id = _ref7.id,
      items = _ref7.items,
      nestedGroupKey = _ref7.nestedGroupKey;
  return items.length ? (0, _core.jsx)(_HeaderSection.default, {
    id: id,
    key: nestedGroupKey
  }, function (_ref8) {
    var className = _ref8.className;
    return (0, _core.jsx)("div", {
      className: className
    }, (0, _core.jsx)(TypedItemsRenderer, {
      items: items,
      customComponents: customComponents
    }));
  }) : null;
};

var MenuSection = function MenuSection(_ref9) {
  var alwaysShowScrollHint = _ref9.alwaysShowScrollHint,
      customComponents = _ref9.customComponents,
      id = _ref9.id,
      items = _ref9.items,
      nestedGroupKey = _ref9.nestedGroupKey,
      parentId = _ref9.parentId;
  return (0, _core.jsx)(_MenuSection.default, {
    alwaysShowScrollHint: alwaysShowScrollHint,
    id: id,
    key: nestedGroupKey,
    parentId: parentId
  }, function (_ref10) {
    var className = _ref10.className;
    return (0, _core.jsx)("div", {
      className: className
    }, (0, _core.jsx)(TypedItemsRenderer, {
      items: items,
      customComponents: customComponents
    }));
  });
};

var SortableContext = function SortableContext(_ref11) {
  var customComponents = _ref11.customComponents,
      id = _ref11.id,
      items = _ref11.items,
      onDragStart = _ref11.onDragStart,
      onDragUpdate = _ref11.onDragUpdate,
      onDragEnd = _ref11.onDragEnd;
  return items && items.length ? (0, _core.jsx)(LazySortableContextComponent, {
    id: id,
    onDragStart: onDragStart,
    onDragUpdate: onDragUpdate,
    onDragEnd: onDragEnd
  }, (0, _core.jsx)(TypedItemsRenderer, {
    items: items,
    customComponents: customComponents
  })) : null;
};

var itemComponents = {
  BackItem: _BackItem.default,
  ContainerHeader: ContainerHeader,
  Debug: Debug,
  GoToItem: _GoToItem.default,
  GroupHeading: GroupHeading,
  Item: _ConnectedItem.default,
  SortableItem: LazySortableItem,
  SectionHeading: SectionHeading,
  Separator: _Separator.default,
  Switcher: LazySwitcher,
  Wordmark: _Wordmark.default
};

var renderItemComponent = function renderItemComponent(props, key, index) {
  var element = null; // We need an explicit conditional against each type for flow type refinement to work

  if (props.type === 'BackItem') {
    var type = props.type,
        compProps = (0, _objectWithoutProperties2.default)(props, ["type"]);
    element = (0, _core.jsx)(_BackItem.default, (0, _extends2.default)({
      key: key
    }, compProps, {
      index: index
    }));
  } else if (props.type === 'ContainerHeader') {
    var _type = props.type,
        _compProps = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(ContainerHeader, (0, _extends2.default)({
      key: key
    }, _compProps));
  } else if (props.type === 'Debug') {
    var _type2 = props.type,
        _compProps2 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(Debug, (0, _extends2.default)({
      key: key
    }, _compProps2));
  } else if (props.type === 'GoToItem') {
    var _type3 = props.type,
        _compProps3 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(_GoToItem.default, (0, _extends2.default)({
      key: key
    }, _compProps3, {
      index: index
    }));
  } else if (props.type === 'Item') {
    var _type4 = props.type,
        _compProps4 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(_ConnectedItem.default, (0, _extends2.default)({
      key: key
    }, _compProps4, {
      index: index
    }));
  } else if (props.type === 'SortableItem') {
    var _type5 = props.type,
        _compProps5 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(LazySortableItem, (0, _extends2.default)({
      key: key
    }, _compProps5, {
      index: index
    }));
  } else if (props.type === 'SectionHeading') {
    var _type6 = props.type,
        id = props.id,
        _compProps6 = (0, _objectWithoutProperties2.default)(props, ["type", "id"]);

    element = (0, _core.jsx)(SectionHeading, (0, _extends2.default)({
      key: key
    }, _compProps6));
  } else if (props.type === 'Separator') {
    var _type7 = props.type,
        _id = props.id,
        _compProps7 = (0, _objectWithoutProperties2.default)(props, ["type", "id"]);

    element = (0, _core.jsx)(_Separator.default, (0, _extends2.default)({
      key: key
    }, _compProps7));
  } else if (props.type === 'Switcher') {
    var _type8 = props.type,
        _compProps8 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(LazySwitcher, (0, _extends2.default)({
      key: key
    }, _compProps8));
  } else if (props.type === 'Wordmark') {
    var _type9 = props.type,
        _id2 = props.id,
        _compProps9 = (0, _objectWithoutProperties2.default)(props, ["type", "id"]);

    element = (0, _core.jsx)(_Wordmark.default, (0, _extends2.default)({
      key: key
    }, _compProps9));
  }

  return element;
};

var groupComponents = {
  Group: Group,
  HeaderSection: HeaderSection,
  MenuSection: MenuSection,
  Section: Section,
  SortableContext: SortableContext,
  SortableGroup: SortableGroup
};

var renderGroupComponent = function renderGroupComponent(props, key, customComponents) {
  var element = null; // We need an explicit conditional against each type for flow type refinement to work

  if (props.type === 'Group') {
    var type = props.type,
        compProps = (0, _objectWithoutProperties2.default)(props, ["type"]);
    element = (0, _core.jsx)(Group, (0, _extends2.default)({
      key: key
    }, compProps, {
      customComponents: customComponents
    }));
  } else if (props.type === 'HeaderSection') {
    var _type10 = props.type,
        _compProps10 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(HeaderSection, (0, _extends2.default)({
      key: key
    }, _compProps10, {
      customComponents: customComponents
    }));
  } else if (props.type === 'MenuSection') {
    var _type11 = props.type,
        _compProps11 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(MenuSection, (0, _extends2.default)({
      key: key
    }, _compProps11, {
      customComponents: customComponents
    }));
  } else if (props.type === 'Section') {
    var _type12 = props.type,
        _compProps12 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(Section, (0, _extends2.default)({
      key: key
    }, _compProps12, {
      customComponents: customComponents
    }));
  } else if (props.type === 'SortableContext') {
    var _type13 = props.type,
        _compProps13 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(SortableContext, (0, _extends2.default)({
      key: key
    }, _compProps13, {
      customComponents: customComponents
    }));
  } else if (props.type === 'SortableGroup') {
    var _type14 = props.type,
        _compProps14 = (0, _objectWithoutProperties2.default)(props, ["type"]);

    element = (0, _core.jsx)(SortableGroup, (0, _extends2.default)({
      key: key
    }, _compProps14, {
      customComponents: customComponents
    }));
  }

  return element;
}; // Exported for testing purposes only.


var components = (0, _objectSpread2.default)({}, itemComponents, groupComponents);
/**
 * RENDERER
 */

exports.components = components;

var TypedItemsRenderer =
/*#__PURE__*/
function (_PureComponent) {
  (0, _inherits2.default)(TypedItemsRenderer, _PureComponent);

  function TypedItemsRenderer() {
    var _getPrototypeOf2;

    var _this;

    (0, _classCallCheck2.default)(this, TypedItemsRenderer);

    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    _this = (0, _possibleConstructorReturn2.default)(this, (_getPrototypeOf2 = (0, _getPrototypeOf3.default)(TypedItemsRenderer)).call.apply(_getPrototypeOf2, [this].concat(args)));
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "customComponentsWithAnalytics", new Map());
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getCustomComponent", function (component) {
      // cache custom components wrapped with analytics
      // to prevent re-mounting of component on re-render
      var _this$props$customCom = _this.props.customComponents,
          customComponents = _this$props$customCom === void 0 ? {} : _this$props$customCom;

      var cachedComponent = _this.customComponentsWithAnalytics.get(component);

      if (!cachedComponent) {
        cachedComponent = typeof component === 'string' ? (0, _analytics.navigationItemClicked)(customComponents[component], component) : (0, _analytics.navigationItemClicked)(component, component.displayName || 'inlineCustomComponent');

        _this.customComponentsWithAnalytics.set(component, cachedComponent);
      }

      return cachedComponent;
    });
    return _this;
  }

  (0, _createClass2.default)(TypedItemsRenderer, [{
    key: "render",
    value: function render() {
      var _this2 = this;

      var _this$props = this.props,
          _this$props$customCom2 = _this$props.customComponents,
          customComponents = _this$props$customCom2 === void 0 ? {} : _this$props$customCom2,
          items = _this$props.items; // We cannot destructure props.type otherwise flow type refinment does not work
      // https://github.com/facebook/flow/issues/5259

      return items.map(function (props, index) {
        var key = typeof props.nestedGroupKey === 'string' ? props.nestedGroupKey : props.id;

        if (props.type === 'InlineComponent') {
          var type = props.type,
              component = props.component,
              componentProps = (0, _objectWithoutProperties2.default)(props, ["type", "component"]); // If they've provided a component as the type

          var CustomComponent = _this2.getCustomComponent(props.component);

          return (0, _core.jsx)(CustomComponent, (0, _extends2.default)({
            key: key
          }, componentProps, {
            index: index // We pass our in-built components through to custom components so
            // they can wrap/render them if they want to.
            ,
            components: components,
            customComponents: customComponents
          }));
        }

        if (Object.keys(groupComponents).includes(props.type)) {
          // If they've provided a type which matches one of our in-built group
          // components
          return renderGroupComponent(props, key, customComponents); // If they've provided a type which matches one of our in-built item
          // components.
        }

        if (Object.keys(itemComponents).includes(props.type)) {
          return renderItemComponent(props, key, index);
        }

        if (Object.keys(customComponents).includes(props.type)) {
          var _type15 = props.type,
              _componentProps = (0, _objectWithoutProperties2.default)(props, ["type"]); // If they've provided a type which matches one of their defined custom
          // components.


          var _CustomComponent = _this2.getCustomComponent(_type15);

          return (0, _core.jsx)(_CustomComponent, (0, _extends2.default)({
            key: key
          }, _componentProps, {
            index: index // We pass our in-built components through to custom components so
            // they can wrap/render them if they want to.
            ,
            components: components,
            customComponents: customComponents
          }));
        }

        return (0, _core.jsx)(Debug, (0, _extends2.default)({
          key: key,
          type: props.type
        }, props));
      });
    }
  }]);
  return TypedItemsRenderer;
}(_react.PureComponent);

var _default = TypedItemsRenderer;
exports.default = _default;